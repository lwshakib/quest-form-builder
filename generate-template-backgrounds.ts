...
async function generateImage(template: any) {
  ...
  if (template.id === "t-shirt-signup")
    styleSpecifics =
      "a stylish boutique t-shirt shop interior, neatly folded colorful t-shirts on wooden shelves, professional lighting";
  else if (template.id === "contact-info")
    styleSpecifics =
      "a modern office desk with a succulent, a clean notebook, and a sleek pen, warm natural light";
  else if (template.id === "rsvp" || template.id === "party-invite")
    styleSpecifics =
      "elegant party decorations, soft bokeh lights, celebratory atmosphere without people";
  else if (template.id === "time-off-request" || template.id === "find-a-time")
    styleSpecifics =
      "a peaceful vacation scene, tropical beach or a cozy mountain cabin, tranquil mood";
  else if (template.id === "order-form" || template.id === "work-request")
    styleSpecifics =
      "organized workspace, architectural tools, or high-end craft supplies on a clean table";
  else if (template.id === "event-registration")
    styleSpecifics =
      "a professional conference hall lobby, modern architecture, sleek registration desk area";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `[ERROR] API failed for ${template.id}:`,
