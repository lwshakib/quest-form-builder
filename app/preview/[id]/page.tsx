  ...
  if (isNotFound) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-muted-foreground mt-2">You must be the owner of this quest to preview it.</p>
        <Button onClick={() => router.push('/quests')} className="mt-8">Go back to Dashboard</Button>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto relative space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000">
        <form onSubmit={handleSubmit} className="space-y-6 pb-20">
              ...
              >
                ...
                <div className="p-6 sm:p-8 space-y-6 relative z-10">
                  ...
                  <div className="space-y-6">
                    ...
                    {q.type === 'MULTIPLE_CHOICE' && (
                      ...
                      >
                        {(q.options as string[] || []).map((option) => (
                          ...
                          >
                            <RadioGroupItem value={option} className="opacity-0 absolute w-0 h-0" />
                            <div className={cn(
                    ...
                    {q.type === 'DROPDOWN' && (
                      <Select onValueChange={(val) => handleInputChange(q.id, val)} value={answers[q.id] || undefined}>
                        ...
                        <SelectContent className="rounded-xl border border-border/60 bg-background shadow-xl">
                          {(q.options as string[] || []).map((option) => (
                            <SelectItem key={option} value={option} className="py-3 text-sm font-medium transition-all cursor-pointer">
                              {option}
                            </SelectItem>
                          ))}
                        </SelectContent>
          ...
          <footer className="pt-12 text-center">
            ...
            <p className="mt-4 text-xs text-muted-foreground">Responses entered in preview mode are not recorded.</p>
          </footer>
        </form>
      </div>
    </div>
