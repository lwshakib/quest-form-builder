  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto py-12 px-4 lg:px-8 space-y-20">
        {/* Categories */}
        {categories.map((cat) => {
          const catTemplates = cat.name === 'Recent' ? recentTemplates : filteredTemplates.filter(t => t.category === cat.name);
            ...
            <section key={cat.name} className="space-y-8">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-bold tracking-tight text-muted-foreground uppercase flex items-center gap-2">
                  <cat.icon className="h-5 w-5" /> {cat.name === 'Recent' ? 'Recently used templates' : cat.name}
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                  ...
                  >
                    <div className="relative w-full aspect-[16/9] rounded-2xl overflow-hidden border-2 border-dashed border-border bg-card hover:bg-primary/5 transition-all duration-500 hover:border-primary/40 shadow-sm hover:shadow-xl flex items-center justify-center">
                      <div className="bg-primary/5 p-3 rounded-full group-hover:scale-110 group-hover:bg-primary/10 transition-all duration-500 border border-primary/10">
                        {isCreating === 'blank' ? <Loader2 className="h-5 w-5 animate-spin text-primary" /> : <Plus className="h-5 w-5 text-primary" />}
                      </div>
                      {isCreating === 'blank' && (
                    ...
                    <div className="space-y-1 px-1">
                      <h3 className="font-bold text-base group-hover:text-primary transition-colors">Blank Quest</h3>
                      <p className="text-xs text-muted-foreground font-medium line-clamp-1">Start from scratch</p>
                    </div>
                  </button>
                )}
                ...
                {catTemplates.map((template) => (
                  ...
                  >
                    ...
                    <div className="space-y-1 px-1">
                      <h3 className="font-bold text-base group-hover:text-primary transition-colors">{template.title}</h3>
